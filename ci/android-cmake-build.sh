...
qt-cmake \
    ...
    -DCMAKE_FIND_ROOT_PATH:PATH=$QT_INSTALL_ROOT/$QT_VERSION/$QT_SUBDIR \
    -DQT_QMAKE_EXECUTABLE:FILEPATH=$QT_INSTALL_ROOT/$QT_VERSION/$QT_SUBDIR/bin/qmake \
    -DCMAKE_PREFIX_PATH:PATH=$QT_INSTALL_ROOT/$QT_VERSION/$QT_SUBDIR \
...
cp src/android-build/OpenTodoList.apk \
    OpenTodoList-${ANDROID_ABIS}-${OTL_VERSION}.apk
cp src/android-build/build/outputs/bundle/release/android-build-release.aab \
