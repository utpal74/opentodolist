...
QtObject {
    ...
    property var appConnections: Connections {
        ...
        function onOpenLinkToLibrary(url, uid) {
            rootObject.d.addTransaction({
                "uid": uid,
                "url": url,
                "transactionId": OTL.Application.loadLibrary(uid),
                "type": "library"
            });
        }
        ...
        function onOpenLinkToItem(url, uid) {
            rootObject.d.addTransaction({
                "uid": uid,
                "url": url,
                "transactionId": OTL.Application.loadItem(uid),
                "type": "item"
            });
        }
        ...
        function onItemLoaded(uid, data, parents, library, transactionId) {
            ...
            if (transaction && transaction.type === "item") {
                ...
                case "Note":
                    ...
                    break;
                case "TodoList":
                    window.deepLinkHandler.openTodoList(transaction.url, item, lib);
                    break;
                case "Todo":
                    window.deepLinkHandler.openTodo(transaction.url, item, OTL.Application.itemFromData(parents[0]), lib);
                    break;
                case "Image":
    ...
    Component.onCompleted: {
        OTL.SyntaxHighlighter.theme = Qt.binding(function () {
            if (C.ColorTheme.isDarkColorScheme) {
