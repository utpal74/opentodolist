...
import net.rpdev.OpenTodoList.Pages
...
Column {
                    ...
                    onTriggered: {
                        if (textEdit.selectedText === "") {
                            OTL.Application.copyHtmlToClipboard(
                                        Markdown.markdownToHtml(root.item?.notes
                                                                ?? "", palette))
                        } else {
    ...
    FileDialog {
        ...
        currentFile: StandardPaths.writableLocation(StandardPaths.DocumentsLocation) + "/notes.md"
        defaultSuffix: ".md"
